...
@Component({
  selector: 'app-show_email_data',
  templateUrl: './show_email_data.component.html',
  styleUrls: ['./show_email_data.component.css']
})
export class ShowEmailDataComponent implements OnInit {
  ...
  public filename=localStorage.getItem("emailFileName")
  loginInfo: Login = {
    user_name: null,
  }
  public leads;
  ...
  showUploadedEmailData() {
    console.log(this.filename)
    this.campaingService.showEmailDataByFileName(this.filename).subscribe(
      data => {
        this.leads = data
  ...
  onChange(event, id) {
    ...
    this.campaingService.updateEmailStatus(id, this.active).subscribe(
      data => {
